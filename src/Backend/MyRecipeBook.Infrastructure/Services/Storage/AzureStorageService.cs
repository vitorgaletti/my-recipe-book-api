...
namespace MyRecepiBook.Infrastructure.Services.Storage;
...
public class AzureStorageService : IBlobStorageService
{
    ...
    public async Task<string> GetFileUrl(User user, string fileName)
    {
        ...
        if (exist.Value)
        {
            var sasBuilder = new BlobSasBuilder()
            {
                ...
                BlobName = fileName,
                Resource = "b",
                ExpiresOn = DateTimeOffset.UtcNow.AddMinutes(MyRecipeBookRuleConstants
                    .MAXIMUM_IMAGE_URL_LIFETIMR_IN_MINUTES)
            };
    ...
    public async Task DelteContainer(Guid userIdentifier)
    {
